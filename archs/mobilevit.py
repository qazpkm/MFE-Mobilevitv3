...
class MobileViT(nn.Module):
    ...
    def _make_mit_layer(input_channel: int, cfg: Dict) -> [nn.Sequential, int]:
        ...
        block.append(MobileViTBlock(
            ...
            ffn_dropout=cfg.get("ffn_dropout", 0),
...
def mobile_vit_xxs(pretrained=True):
    model = mobile_vit_xx_small(num_classes=3)
    device = torch.device('cuda:0' if torch.cuda.is_available() else "cpu")
    weights = 'E:/ACTon/ACTON/action/ACTION-Net-main/archs/mobilevit_xxs.pt'
    if pretrained:
        try:
