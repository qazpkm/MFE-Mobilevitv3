...
def inference(model, val_dataloader):
    ...
    with torch.no_grad():
        for step, inputs in enumerate(tqdm(val_dataloader)):
            ...
            prec1, prec5 = accuracy(outputs.data.mean(1), labels, topk=(1, 2))
...
if __name__ == '__main__':
    ...
    checkpoint_path = '{}-{}/2023-05-15-23-35-21/clip_len_{}frame_sample_rate_1_checkpoint.pth.tar'.format(args.dataset, args.base_model,
                                                                                                                 args.clip_len)
    ...
    model.load_state_dict({k.replace('module.', ''): v for k, v in pretrained_dict['state_dict'].items()})
    model.load_state_dict(pretrained_dict['state_dict'])
    # img_path = 'data/img23.jpg'
    # img_size = 224
    ...
    if args.dataset == 'jester':
        ...
        val_dataloader = DataLoader(dataset_ucf101.dataset_video_inference(annot_path, 'val', clip_num=args.clip_num,
                                                                                spatial_transform=spatial_transform,
                                                                                temporal_transform = temporal_transform),
                                    batch_size=args.batch_size,
                                    num_workers=args.num_workers)
