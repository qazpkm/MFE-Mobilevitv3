...
from data import dataset_jester, dataset_EgoGesture, dataset_sthv2, dataset_ucf101
...
def parse_opts():
    ...
    parser.add_argument('--is_shift', default=1, action="store_true")
    parser.add_argument('--npb', action="store_true")
    parser.add_argument('--pretrain', type=str, default='imagenet')  # 'imagenet' or False
    # parser.add_argument('--pretrain', type=str, default='E:/ACTon/ACTON/action/ACTION-Net-main/archs/pre_efficientnetv2-s.pth')
    parser.add_argument('--dropout', default=0, type=float)
    parser.add_argument('--base_model', default='mobilevit', type=str)
...
annot_path = 'E:/ACTon/ACTON/action/ACTION-Net-main/datafeed/ucf101/'
label_path = 'E:/ACTon/ACTON/action/ACTION-Net-main/datafeed/RES/{}/'.format(args.dataset)  # for submitting testing results
...
def recall(output, target, topk=(1,)):
    ...
    target = target.detach().numpy()
    predict.extend(pred2)
    turedict.extend(target)
    print('pre:', predict)
    print('ture', turedict)
    
    pre = accuracy_score(turedict, predict)
    rec = recall_score(turedict, predict, average=None)
    prc = precision_score(turedict, predict, average=None)
    return rec, pre, prc
...
def train(model, train_dataloader, epoch, criterion, optimizer, writer):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        if args.dataset == 'EgoGesture' or args.dataset == 'nvGesture':
            ...
            #outputs = model(rgb)
            #print("78888888888888888888888888888888888")
        else:
            rgb, labels = inputs[0], inputs[1]
            #print(rgb)
            rgb = rgb.to(device, non_blocking=True).float()
            outputs = model(rgb)
            # print("2314654694789777777777")
            # print(outputs)
            # text_save(apath,'131321321132123321231321111111111111111132131')
            # bbout = softmax(outputs)
            # text_save(apath,bbout)
            # text_save(apath,'131321321132123321231321111111111111111132131')
        labels = labels.to(device, non_blocking=True).long()
        loss = criterion(outputs, labels)

        # measure accuracy and record loss
        # rec, pre, prc = recall(outputs.data, labels, topk=(1, 1))
        # print('recall:', rec)
        # print('pre:', pre)
        # print('precision:', prc)
...
def main():
    ...
    if args.is_train:
        ...
        if args.dataset == 'EgoGesture':
            ...
            train_dataset = dataset_ucf101.dataset_video(annot_path, 'train', spatial_transform=trans_train,
                                                         temporal_transform=temporal_transform_train)
            train_dataloader = DataLoader(train_dataset, batch_size=params['batch_size'], shuffle=True,
                                          num_workers=params['num_workers'])
            val_dataset = dataset_ucf101.dataset_video(annot_path, 'val', spatial_transform=trans_test,
                                                       temporal_transform=temporal_transform_test)
            val_dataloader = DataLoader(val_dataset, batch_size=params['batch_size'], num_workers=params['num_workers'])
        print("load model")
        ...
        flops, param = get_model_complexity_info(model, (4, 8, 3, 224, 224), as_strings=True, print_per_layer_stat=True)
        print('Flops:  ' + flops)
        print('Params: ' + param)
