...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
        ...
        self.weights = 'E:/ACTon/ACTON/action/ACTION-Net-main/archs/pre_efficientnetv2-s.pth'
        self.num_classes = 3
        self.device = 'cuda:0'
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet50' in base_model:
            ...
            from archs.efficientnetv2 import efficientnetv2_s, EfficientNetV2, MBConv, FusedMBConv,efficientnetv2s
            self.base_model = efficientnetv2s(True if self.pretrain == 'imagenet' else False)
            self.base_model.last_layer_name = 'classifier'

            self.input_size = [224, 224]
            self.img_size = [224, 224],  # train_size, val_size

            self.input_mean = [0.485, 0.456, 0.406]
            self.input_std = [0.229, 0.224, 0.225]
            """device = torch.device(self.device if torch.cuda.is_available() else "cpu")
            model = efficientnetv2_s(num_classes=self.num_classes).to(device)
            if self.pretrain != "":
        ...
        elif base_model == 'mobilevit':
            from archs.mobilevit import mobile_vit_xxs, InvertedResidual, MobileViT, ConvLayer
            self.base_model = mobile_vit_xxs(True if self.pretrain == 'imagenet' else False)
            ...
            if self.is_shift:
                ...
                for m in self.base_model.modules():
                    if isinstance(m, InvertedResidual):
                        #print(m.block)
                        #print(m.block[0])
                        #print('122121212121212121212121212111111111111')

                        for nmnm in m.block:
                            #print(nmnm.exp_1x1)
                            #print(nmnm)
                            #print(len(nmnm.block))
                            #print('122121212121212121212121212111111111111')
                            if nmnm == m.block[0]:
                                if isinstance(nmnm, ConvLayer)and len(nmnm.block) == 3 and m.use_res_connect:
                                    nmnm.block[0] = Action(nmnm.block[0], n_segment=self.num_segments, shift_div=self.shift_div)
                                    #print('SUCCESS')

                                #print(len(nmnm.block))
                                #print(nmnm.block[0])
                        #print(m.use_res_connect)
                # if isinstance(m, InvertedResidual) and len(m.block) == 3 and m.use_res_connect:
                #         print(m.block)
                #         print(m.use_res_connect)
                #         m.block[0] = Action(m.block[0], n_segment=self.num_segments, shift_div=self.shift_div)
                else:
                    aaSASDASD = 1
                    #print('error')
            ...
            if self.modality == 'Flow':
                ...
                self.input_mean = [0.485, 0.456, 0.406] + [0] * 3 * self.new_length
                self.input_std = self.input_std + [np.mean(self.input_std) * 2] * 3 * self.new_length
            print('TotalAAAAA params: %.2fM' % (sum(p.numel() for p in self.base_model.parameters()) / 1000000.0))
        ...
        for name, m in self.named_modules():
            if 'action' in name:
                ...
                if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d) or isinstance(m, torch.nn.Conv3d):
                    ...
                    a = 1
                elif isinstance(m, torch.nn.TransformerEncoder):
    ...
    def forward(self, input, no_reshape=False):
        ...
        apath = 'E:/ACTon/ACTON/action/ACTION-Net-main/selimg.txt'
        if not no_reshape:
        ...
        #print("+645684154156314563143561")
        # bout = []
        # bbout = self.ssoftmax(base_out)
        # pred2 = bbout.data.cpu()
        # target = pred2.detach().numpy()
        # bout.append(target)
        # # bout.append('1111111111111111111111111111111111111111111111111111111')
        # text_save(apath,bout)
        if self.dropout > 0:
